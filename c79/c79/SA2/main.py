...
background_image = pygame.image.load("C:/Users/arnav/Downloads/c79/c79/SA2/track.png").convert()
player_image = pygame.image.load("C:/Users/arnav/Downloads/c79/c79/SA2/car.png").convert_alpha()
...
def getSensorsData(car, angle):
    ...
    # Create an empty list sensorData
    sensorData = []
    for sensorAngle in sensorAngles:
        newX = getSensorX(x, angle, sensorAngle, margin)
        newY = getSensorY(y, angle, sensorAngle, margin)      

        # Use checkPixel to get the color information at newX, newY and append it to sensorData
...
def eval_fitness(generation, config):
    ...
    for gid, genome in generation:
        ...
        while True:
          ...
          pygame.draw.rect(screen,(0,0,0), [400, 0, 210, 65])
          ...
          # Show the all sensors and ouput data on the screen Boiler
...
config = neat.config.Config(neat.DefaultGenome, neat.DefaultReproduction,neat.DefaultSpeciesSet, neat.DefaultStagnation,
                            'C:/Users/arnav/Downloads/c79/c79/SA2/config-feedforward.txt')  
